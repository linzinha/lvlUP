...
def addPointsP1():
    ...
    addPGreeting = tk.Label(text="Add Points for Player One:", bg="mediumturquoise",
                            fg="black", font="10", width=50, pady=20)
    ...
    def addNumbers():
        ...
        # exercise Points e5
        try:
            subtractBMR = int(e2.get()) - 1420
            exSub.set(subtractBMR)
            exercises = math.floor((int(e2.get()) - 1420) * .02)
    ...
    def finalTally():
        ...
        pKeys = p1statListWeekScores.keys()
        pointCategories = []
        ...
        for cat in pointCategories:
            p1statListWeekScores[cat] += dailyTally[i]
            p1statListMonthScores[cat] += dailyTally[i]

            i += 1

        p1Totals['total week'] += totalPointsEarned
        p1Totals['total month'] += totalPointsEarned
